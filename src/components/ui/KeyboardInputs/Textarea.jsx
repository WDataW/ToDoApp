...
export default function Textarea({ className = "", children, ...props }) {
    ...
    return (
        <div className={`${className} flex flex-col`}>
            ...
            <textarea id={id} className={`w-full outline-none py-[0.4rem] px-[0.8rem] placeholder-opacity-50 outline-none border-s min-h-[5rem] rounded-[0.4rem]  ${themeStyles[theme]}`} {...props}>
                {children}
            </textarea>
        </div>
    );
