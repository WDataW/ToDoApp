...
import { useTheme, gradientColors } from "@/context/Theme";
const icons = {
    light: "bg-[url(/src/assets/icons/light/calendar.svg)]",
    dark: "bg-[url(/src/assets/icons/dark/calendar.svg)]"
}
export default function TasksCalendar({ className = "", children, ...props }) {
    ...
    return (
        <div className={`${className}`} {...props}>
            <SectionContainer className={`   border justify-center mb-[0.9rem]  `}>
                <h3 className="capitalize border-b mb-[0.3rem] mx-[0.4rem]"><span className={`h-[1rem] me-[0.3rem] bg-size-[1.1rem_1.1rem] w-[1rem] ${icons[theme]} bg-cover bg-center bg-no-repeat inline-block`}></span>{t("terms.calendar")}</h3>
            ...
            <AnimatePresence mode="wait">
                <motion.div className="mb-[1rem]" key={day} exit={{ y: 25, opacity: 0 }} initial={{ y: 25, opacity: 0 }} animate={{ y: 0, opacity: 1 }} >
