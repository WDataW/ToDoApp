...
export default function NavItem({ className = "", children, image, size, active = false, ...props }) {
    ...
    return (
        <li className="sm:w-full">
            <a href={null} className={` p-0  flex flex-col justify-center items-center  sm:px-[1rem] sm:py-[0.5rem] py-[0.5rem] px-[0.5rem] ${active && activeSurfaceBgColor + " border-t-[0.1rem] border-s-0 sm:border-t-0 sm:border-s-[0.15rem]"}   ${className}`}>
