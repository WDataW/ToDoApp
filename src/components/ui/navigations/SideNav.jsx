...
import { SettingsAnchor, InboxPage } from "../../ui";
...
export default function SideNav({ selfRef, viewInbox, setViewInbox, title, ...props }) {
    ...
    return (
        <MainNav invert={true} title={viewInbox || title} ulClassName="items-center flex-col h-full gap-[0.3rem] py-[1rem]" className="fixed start-0 top-0 h-full w-[4.7rem]" {...props}>
            <li ref={selfRef} className="w-full">
                <button onClick={startViewingInbox} href={null} className={`w-full cursor-pointer p-0 flex flex-col justify-center items-center  sm:px-[1rem] sm:py-[0.5rem] py-[0.5rem] px-[0.5rem] ${viewInbox && activeSurfaceBgColor + " border-t-[0.1rem] border-s-0 sm:border-t-0 sm:border-s-[0.15rem]"}`}>
