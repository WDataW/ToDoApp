...
export default function HeroSection({ className = "", children, ...props }) {
    ...
    return (
        <div className={`${className} border-b relative flex  md:flex-col    items-end  pt-[7.5rem] md:pt-[3rem] md:py-0 h-[65vh]  md:h-screen`} {...props}>
            ...
            <div className={` text-center   flex flex-col items-center md:block  md:text-start absolute  top-[14rem] w-full left-1/2 -translate-x-1/2 -translate-y-1/2 ${paddingX}`}>
                ...
                <Link to="/auth/sign-up" className={`transition-[scale] hover:scale-110 border mt-[2rem] ${theme == "dark" ? "bg-white text-black" : "bg-black text-white"} flex items-center justify-center rounded-[1rem]  border h-[2.6rem] w-[8rem]`}>{t("terms.getStarted")}</Link>
            </div>
        </div>
    );
}