...
export default function SettingButton({ active, label, itemStyle, className = "", children, ...props }) {
    ...
    const icons = {
        dark: "bg-[url(/src/assets/icons/dark/arrowDown.svg)] ", light: "bg-[url(/src/assets/icons/light/arrowDown.svg)] "
    }

    return (<div style={{ height: active && itemStyle ? "5.3rem" : "" }} className={`h-[2.3rem] overflow-hidden transition-[height] flex flex-col items-center  justify-start  ${itemStyle || commonStyles}`}>
